...
class TestResidentTypeAPI:
    ...
    def test_each_type_has_required_fields(self, api: APIClient):
    ...
    def test_create_resident_type(self, api: APIClient):
        ...
        if type_id:
            api.delete_resident_type(type_id)

    def test_create_type_is_active_by_default(self, api: APIClient):
    ...
    def test_update_resident_type(self, api: APIClient):
        ...
        result = api.update_resident_type(type_id, {
            "name":        updated_name,
    ...
    def test_delete_resident_type(self, api: APIClient):
        ...
        result = api.delete_resident_type(type_id)
        assert result is True

        # Verify: should be 404 or marked deleted
        resp = api.session.get(f"{API['resident_types']}{type_id}/", timeout=10)
    ...
    def test_each_seeded_type_by_id(self, api: APIClient, type_name: str, type_id: int):
        """Parametrized: verify each of the 8 seeded types by exact ID."""
        t = api.get_resident_type(type_id)
...
class TestResidentTypeUI:
    ...
    def test_all_types_visible_in_ui(self, auth_page: Page, api: APIClient):
